...
private final class PythonUDFRunner(
  requests: Queue[PythonUDFRunner.Request]
) extends UDFRunner {
  import PythonUDFRunner.{Request, toJava, fromJava}
    ...
    Queue.unbounded[Request].toManaged(_.shutdown).flatMap { requests =>
      Sys.extractResource("/start-server.py").flatMap { startServer =>
        ...
        ZManaged.collectAllPar_(List.fill(workers)(startWorker.fork)).as {
          new PythonUDFRunner(requests)
        }
      }
    }.toLayer

  private final case class Request(function: String, input: Any, cb: Promise[Throwable, Any])
